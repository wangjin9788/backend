...
public class InitGroupAndManufacturerExcelTools {

  protected static final Logger logger =
      LoggerFactory.getLogger(InitGroupAndManufacturerExcelTools.class);

  public static List<String[]> readExcelByPoi(final String filePath,
      final int sheetNumber, final int columnSize, final int mark)
      throws Exception {
    Workbook wb = null;
    ...
    try {
      ...
      while (rows.hasNext()) {
        ...
        if (row.getRowNum() == 0) {
          if (mark == 1) {
            if (!"供應商".equals(cell0.trim()) || !"供應商全稱".equals(cell1.trim())) {
              logger.error("Excel header does not match");
              break;
            }
          } else {
            if (!"END USER".equals(cell0.trim())
                || !"客戶名稱".equals(cell1.trim())) {
              logger.error("Excel header does not match");
              break;
            }
          }
        ...
        if (Strings.isNullOrEmpty(cell0)) {
          return resultList;
        }
