 ...
 *
 */
public class GenerateSaCommodityPurchaseAnalysis implements Runnable {

  private static final Logger logger =
      LoggerFactory.getLogger(GenerateSaCommodityPurchaseAnalysis.class);

  private final Date eDate;
  private final Date lDate;
  private final SaCommodityPurchaseService saCommodityPurchaseService;

  public GenerateSaCommodityPurchaseAnalysis(final Date eDate, final Date lDate,
  ...
  public void run() {
    ...
    while (eCalendar.compareTo(lCalendar) <= 0) {
      ...
      saCommodityPurchaseService.generateSaCommodityPurchaseForMonth(currentYear, currentMonth, null, null, null);
